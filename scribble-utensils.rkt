...
@(require
  ...
  racket
  "coroutines.rkt"
  (for-label "coroutines.rkt" racket)
  (for-template "coroutines.rkt" racket)
  (for-syntax racket))
...
@(define-syntax-rule (Interaction x ...)
   (interaction #:eval (make-base-eval #:lang '(begin (require racket "coroutines.rkt")
                                                         (print-as-expression #f))) x ...))

@(define-syntax-rule (Interaction* x ...)
                     (interaction #:eval evaller x ...))

@(define (make-evaller) (make-base-eval #:lang '(begin (require racket "coroutines.rkt")
